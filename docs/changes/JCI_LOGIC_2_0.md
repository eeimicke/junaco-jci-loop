...
## Ergänzende Umsetzung vom 12.09.2026

Der nachträglich gefundene Identitätsfehler bei vollständigen Beziehungskorrekturen ist behoben. `CORRECTION` und `CLARIFICATION` vergleichen die Beziehung auch nach einer früheren `ADDITION` mit der unmittelbar vorherigen wirksamen Sicht. Richtung, Beziehungstyp, Gegenentitäts-ID und Gegenentitätstyp bleiben erhalten, auch innerhalb gespeicherter Ablösungsketten. Neue Regressionen prüfen verbotene Identitätswechsel und zulässige Änderungen von Beziehungseigenschaften; ursprüngliche PiH und Hashes bleiben unverändert.

Der neue [Neo4j-Transaktionskoordinator](../implementations/neo4j/JCI_NEO4J_RUNTIME.md) setzt den gemeinsamen Gate, dauerhafte Aufträge und Runs, Fencing, ausdrückliche Wiederholungsplanung sowie atomare Erfolgsbelege und Outbox um. Abschließend geprüfte Lese- und Änderungsnachweise werden gebunden; vorhandene Eigentümer dürfen nicht als Neuanlagen behandelt werden. Ein nachgewiesener Worker-Abbruch wird zuerst unter derselben Sperre aufgeklärt und dokumentiert, bevor ein neuer Versuch eingeplant wird.

Die [Integrationstests](../../tests/integration/test_neo4j_transactions.py) verwenden eine isolierte lokale Neo4j-Instanz und ein ausdrücklich begrenztes Test-Regelpaket. Sie prüfen reale Konkurrenz, revisionsneutrale und zuvor fehlende Entscheidungsdaten, Revisions- und Paketkonflikte, zeitlichen Ablauf, Rollback, verlorene Antworten, Wiederholungen und einen Worker-Prozessabbruch vor Commit. [Docker Compose](../../compose.neo4j-test.yml) und ein eigener [CI-Job](../../.github/workflows/validate.yml) machen diesen Testweg reproduzierbar. Der historische Prüfstand oben bleibt davon unberührt.
