...
- dimensionierte Werte (`CiV` – **Core Influential Values**)
...
| `CiV`           | eigener Elementtyp  | einzelner, dreidimensional beschriebener Wert                             |
...
└── gespeicherte JCIElementInstance:
    CiV „Klarheit“ mit NOT-, SELF- und TO-SERVE-Dimension
...
Vorhandene `JCIEntity`-Instanzen sind historisierbar, ausgenommen `PiH`, `ChangeEvent`, `SyncEvent` und `HistoricalCorrection`. Ihre Eigenschaften und ihre eigene zugeordnete Provenienz bleiben unveränderlich. Zulässige Bezugnahmen, die einem anderen neuen Objekt zugeordnet sind, folgen Abschnitt 2.2.8 und schreiben den referenzierten Datensatz nicht um. `TRIGGERS` wird beim Laufabschluss append-only ergänzt; ein erfolgreiches `CREATED` stellt genau ein `CHANGED_BY` von der neuen Entität her. Diese Verknüpfungen verändern weder `revision` noch `updatedAt` des bestehenden `ChangeEvent`. Bei erstmaliger Erzeugung existiert kein vorheriger Zustand und entsteht kein `PiH` der neuen Entität.
...
Bei der Erzeugung gilt `revision = 1` und `updatedAt = createdAt`. Eine unabhängig angeforderte oder beobachtete fachliche Änderung einschließlich eines Statuswechsels startet ein `ChangeEvent` und einen Synchronisationslauf. Deterministische Folgeänderungen bleiben ohne rekursives `ChangeEvent` im selben Auftrag. Jede bestehende Entität, deren Eigenschaften oder zugeordneter Beziehungszustand sich nach Abschnitt 2.2.8 tatsächlich ändern, erhält je erfolgreichem Auftrag genau ein `PiH`, eine Revisionserhöhung und ein neues `updatedAt`. Lediglich betroffene Entitäten und Referenzziele von Auditbeziehungen behalten ihre fachliche Revision. Reine Lesevorgänge verändern weder `updatedAt` noch `revision`.
...
Ein Statuswechsel ist nur zulässig, wenn er in der folgenden Tabelle aufgeführt ist. `SYNC` prüft zusätzlich alle typspezifischen Bedingungen, Beziehungen und anwendbaren `RaN`. Der Eintrag `Erzeugung` bezeichnet den erstmaligen Zustand einer neuen Entität; dabei entsteht kein `PiH` dieser neuen Entität.
...
| `Task`                                                    | `DRAFT` → `BLOCKED`                   | `CHANGED`                 | ausdrückliche Freigabe, vollständige Aktivierungsprüfung und eigene oder aggregierte Blockade nach 9.4.2 |
...
**Kurzes Beispiel:** Ein Task wird als `DRAFT` angelegt. Nach nachgewiesener menschlicher Genehmigung prüft `SYNC` Team, ausführende Rollenaktivierung und sämtliche weiteren Bedingungen. Sind auch die Voraussetzungen erfüllt, wird er `ACTIVE`; bei noch offenen Abhängigkeiten bleibt die Freigabe wirksam, der Task wird jedoch `BLOCKED`. Fällt später eine Voraussetzung weg, setzt `SYNC` ihn ebenfalls auf `BLOCKED`. Sobald sie wieder erfüllt ist, darf er erneut `ACTIVE` werden. Nach bestätigter Ausführung wird er `COMPLETED`; dieser Zustand wird nicht wieder geöffnet.
...
`APPROVED_BY` bezeichnet dagegen eine nachweisbar menschliche, für den konkreten Auftrag autorisierte Genehmigung nach Abschnitt 12.9. Bei einem angenommenen freigabepflichtigen Auftrag ist mindestens eine solche Beziehung erforderlich. Antragsteller und Genehmiger werden getrennt dokumentiert; `REQUESTED_BY` wird nicht auf den Genehmiger umgebogen. Jede `APPROVED_BY`-Kante besitzt `receiptId: UUID`, `decidedAt: DateTime`, `requestHash: String` und `approvalHash: String`. Je Ereignis und Rollenaktivierung besteht höchstens eine Kante. Sie gehört ausschließlich zum bei Annahme erzeugten `ChangeEvent` und darf später weder ergänzt noch verändert oder entfernt werden.
...
| Beziehungskontext                                                                                                                                | Zugeordneter fachlicher Zustand                                                                                            |
| ------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `Verification`: `EVALUATES`, `CHECKS`, `USES_EVIDENCE`, `SUPERSEDES`                                                                             | die neue `Verification`; keine Revision der referenzierten Ziele                                                           |
| `HistoricalCorrection`: `CORRECTS`, `CAUSED_BY`, `CORRECTED_BY`, `USES_EVIDENCE`, `SUPERSEDES`                                                   | die neue `HistoricalCorrection`; keine Revision der referenzierten Ziele                                                   |
| `ChangeEvent`: `REQUESTED_BY`, `APPROVED_BY`, `TARGETS_HISTORY`, `USES_EVIDENCE`                                                                 | das bei Annahme erzeugte `ChangeEvent`; keine Revision der referenzierten Ziele                                            |
| `SyncEvent`: `EXECUTES`, `AFFECTS`, `CREATES_HISTORY`, `CREATES_CORRECTION`                                                                      | das neue `SyncEvent`; keine Revision der referenzierten Ziele                                                              |
| `TRIGGERS`, `CHANGED_BY`, `HAS_HISTORICAL_STATE`                                                                                                 | das jeweilige neue Zielobjekt `SyncEvent`, `ChangeEvent` oder `PiH`; keine weitere Revision allein durch Dokumentation     |
| `CREATED_BY`                                                                                                                                     | die erzeugte Entität; beim zulässigen Nachtragen ihr bestehender Importentwurf; niemals das referenzierte `RoleAssignment` |
| `RaNConflict`: `CONFLICTING_RULE`, `AFFECTS`, `DETECTED_BY`, `USES_EVIDENCE`, `RESOLVED_BY`, `RESOLVED_THROUGH`                                  | der Konflikt; dessen Auflösung erzeugt genau eine Revision und ein `PiH`, nicht Revisionen seiner Referenzziele            |
| `PiH` — `PROVIDES_CONTEXT_TO` — `CiV`                                                                                                            | das `CiV`, dessen historischer Kontext ausgewählt wird; niemals das `PiH`                                                  |
...
Ein Eigenschaftenpfad adressiert genau einen vollständigen `TypedValue`; `<property>` muss zum jeweiligen Snapshot-Typ und Profil passen. Ein ganzer Beziehungseintrag wird als `TypedValue` mit `valueType = OBJECT` übertragen, dessen `value` den vollständigen `RelationshipSnapshot` enthält. Dessen Typ, Richtung, Gegenentitäts-ID und Gegenentitätstyp müssen zum Schlüssel und zulässigen Beziehungskontext passen. Bei einer Berichtigung bleiben diese Identitätsmerkmale unverändert. Nicht erlaubt sind Wurzelersatz, Arrayindizes, Wildcards, `-`, `/stateData/name` oder weitere Unterpfade innerhalb eines `TypedValue`, etwa `/value`. Eine neue Identität darf nicht durch Umschreiben eines alten Beziehungsschlüssels entstehen.

Nach dem JSON-String-Decoding wird der Pointer an `/` segmentiert. Innerhalb eines Segments werden `~1` zu `/` und danach `~0` zu `~` aufgelöst; andere oder unvollständige Tilde-Escapes sind ungültig. Es erfolgt weder URI-Prozent-Decoding noch Unicode-Normalisierung. Zur kanonischen Ausgabe wird zuerst `~` als `~0`, danach `/` als `~1` kodiert. `correctedFields` enthält die eindeutigen kanonischen Pointer in Unicode-Codepunkt-Reihenfolge. Zwei Pfade überlappen, wenn ihre dekodierten Segmentfolgen gleich sind oder eine echte Vorfahr-/Nachfahrbeziehung bilden. Ein bloßes Stringpräfix reicht nicht: `name` und `nameLong` sind verschiedene Felder. Ein ganzer Beziehungseintrag und eine seiner Eigenschaften überlappen dagegen.
...
```text
SHA-256(UTF8(canonicalJSON({stateData, relationshipData})))
relationshipData: sortiert nach (relationshipType, direction, otherEntityId)
Ausgabe: 64 hexadezimale Kleinbuchstaben
```

`canonicalJSON` verwendet UTF-8 ohne BOM und ohne bedeutungslose Leerzeichen; Objektschlüssel sind rekursiv nach Unicode-Codepunkten sortiert. Andere Arrays behalten ihre fachliche Reihenfolge. Anführungszeichen und Backslash verwenden `\"` und `\\`; die Steuerzeichen U+0008, U+0009, U+000A, U+000C und U+000D verwenden `\b`, `\t`, `\n`, `\f` und `\r`. Andere Steuerzeichen U+0000–U+001F verwenden `\u00xx` mit hexadezimalen Kleinbuchstaben. `/` wird nicht maskiert; andere Unicode-Skalarwerte werden unverändert UTF-8-kodiert. Einzelne Surrogates und doppelte JSON-Objektschlüssel sind ungültig. Ganzzahlen werden exakt, ohne feste Größenbegrenzung und ohne Umwandlung in binäre Gleitkommazahlen serialisiert. `DECIMAL` ist eine Zeichenkette nach `-?(0|[1-9][0-9]*)(\.[0-9]*[1-9])?`; `-0`, Exponenten, führende Nullen und nachgestellte Dezimalnullen sind unzulässig. Rohe gebrochene JSON-Zahlen innerhalb von `OBJECT` oder `ARRAY` sind in diesem Profil unzulässig; Dezimalwerte benötigen dort ebenfalls eine ausdrücklich typisierte kanonische Darstellung. Nicht endliche Zahlen sind ungültig. Es findet keine stille Rundung statt. IDs, Hashfelder, Laufzeiten oder Provenienz außerhalb von `stateData` und `relationshipData` werden nicht in eine zusätzliche Hashhülle aufgenommen. Das unveränderte Inhaltsobjekt ohne wirksame Korrektur besitzt denselben Hash für Snapshot und historische Sicht.

**Kurzes Beispiel:** `/relationshipData/INCOMING:HAS_MEMBER:00000000-0000-4000-8000-000000000002` und derselbe Pfad mit `/properties/validFrom` überlappen. Zwei Korrekturen dieser Pfade dürfen nicht gleichzeitig aktiv sein. `/stateData/properties/name` und `/stateData/properties/description` dürfen dagegen unabhängig korrigiert werden.
...
Bevor der neue Zustand übernommen wird, hält der Synchronisationslauf den bisherigen Zustand des betroffenen Elements einschließlich seines nach Abschnitt 2.2.8 zugeordneten, zu diesem Zeitpunkt gültigen Beziehungszustands als eigenes `PiH` fest. Anschließend gilt der geänderte Zustand als aktueller Zustand. Ein zusätzlicher `HistoricalSnapshot` ist nicht erforderlich.
...
Task
  ...
  ◄── INSCRIBES_PURPOSE_IN ── CiV
                                      └── HELD_BY ──► RoFOrg, RoFTeam oder menschliches RoFTeamMember
```
...
Das Hinzufügen, Entfernen oder Ändern einer Beziehung verändert ausschließlich den fachlichen Zustand der nach Abschnitt 2.2.8 zugeordneten Entitäten. Jeder vorhandene veränderliche Eigentümer mit tatsächlichem Delta erhält im Auftrag genau eine Revisionserhöhung, ein neues `updatedAt` und ein `PiH`. Neue Entitäten beginnen mit Revision `1`; Auditziele ändern sich nicht allein durch eine neue Bezugnahme. `SYNC` traversiert dennoch beide Endpunkte und schützt die gesamte relevante Graphsicht einschließlich revisionsneutraler Änderungen.

**Kurzes Beispiel:** Der Task „API bereitstellen“ gehört zum operativen Ziel „Antwortzeit unter 24 Stunden“. Dieses trägt über taktische und strategische Zustände zum langfristigen Ziel bei, das durch die organisationsbezogenen CiV „Klarheit“ und „Verantwortung“ begründet ist. Ausgeführt wird der Task durch Annas Developer-Rollenaktivierung im verantwortlichen Team. Dadurch sind Wertebezug, Scope und Verantwortung vom Task aus vollständig nachvollziehbar.
...
5. Ein `PiH` und sein eigener Snapshotzustand sind unveränderlich und werden niemals erneut historisiert. Anderen Objekten zugeordnete Bezugnahmen nach Abschnitt 2.2.8 verändern das `PiH` nicht.
6. Eigenschaften und eigene zugeordnete Provenienz von `ChangeEvent`, `SyncEvent` und `HistoricalCorrection` bleiben unveränderlich. Ausdrücklich erlaubte append-only Ergänzungen und fremde Bezugnahmen nach Abschnitt 2.2.8 lösen keine Revision oder Historisierung des referenzierten Datensatzes aus.
7. Bei `CREATED` müssen Ziel-ID frei und `requestedRevision = null` sein. Nur `SUCCESS` erzeugt das neue Ziel mit `revision = 1`, `CREATED_BY` und genau einem `CHANGED_BY`. Für dieses neue Ziel entsteht kein `PiH`; vorhandene veränderliche Eigentümer, deren fachliche Beziehungen sich dabei ändern, erhalten nach Abschnitt 2.2.8 ein eigenes `PiH`. `CONFLICT` oder `FAILED` übernimmt weder das Ziel noch ein Fachdelta oder Historie. Angenommenes `ChangeEvent` und `SyncEvent` des beendeten Versuchs bleiben nachvollziehbar.
...
HistoricalCorrection
  ...
  ├── correctedFields: /relationshipData/INCOMING:HAS_MEMBER:00000000-0000-4000-8000-000000000002
  ├── previousValue: NULL
  ├── correctedValue: OBJECT mit vollständigem RelationshipSnapshot zu Team B
...
`CiV` steht für **Core Influential Values**. Jede gespeicherte `CiV`-Instanz beschreibt genau einen grundlegenden Wert, beispielsweise Klarheit oder Verantwortung, aus drei untrennbaren Dimensionen. Sie beantwortet die Frage:

> Welcher Wert gibt einer bestimmten Zukunft Richtung, und für wen gilt er?

`CiV` gibt Zukunftszuständen eine begründete Richtung. Es ist selbst weder ein Zukunftszustand noch eine Aufgabe, Regel oder Rolle. Ein zusätzlicher formulierter Zweck wird nicht am `CiV` gespeichert; er wird bei der Erarbeitung eines `PiF2` aus den verbundenen Werten sichtbar.

### 4.2 Entstehung

Ein `CiV` entsteht, wenn ein einzelner Wert ausdrücklich in drei Dimensionen beschrieben und genau einem Werteträger zugeordnet wird:

```text
NOT CiV      = Was widerspricht diesem Wert und wird ausgeschlossen?
SELF CiV     = Wie zeigt sich der Wert in eigener Identität und Haltung?
TO SERVE CiV = Wem oder was dient der Wert, und welchen Beitrag leistet er?
```

Die drei Pflichtfelder `notCiV`, `selfCiV` und `toServeCiV` bilden diese Dimensionen strukturiert ab. Der gemeinsame Werteträger wird nicht als Freitext gespeichert, sondern über genau eine Beziehung `HELD_BY` zu einer `RoFOrg`, einem `RoFTeam` oder einem menschlichen `RoFTeamMember` festgelegt. Ein technisches `RoFTeamMember` besitzt kein persönliches Wertkonstrukt.

Organisationen, Teams und Menschen können eigene, auch gleich benannte CiV besitzen. Diese bleiben eigenständige Entitäten, weil ihre drei Dimensionen und ihre historische Entwicklung unterschiedlich sein können. Ein Team kann zusätzliche teambezogene Werte erarbeiten. Organisationswerte werden durch das dafür verantwortliche Team und handelnde `RoleAssignments` erarbeitet; `CREATED_BY`, der auslösende `ChangeEvent` und dessen `REQUESTED_BY` erhalten die Erstellungs- und Antragsprovenienz. Die menschliche Genehmigung wird zusätzlich über `APPROVED_BY` und den nachprüfbaren Beleg nach Abschnitt 12.9 festgehalten.

Geht ein Team- oder Organisationswert nach ausdrücklicher fachlicher Entscheidung auf andere CiV zurück, wird diese Herkunft mit `INFORMED_BY` gespeichert. Eine Namensgleichheit, Teammitgliedschaft oder organisatorische Zugehörigkeit erzeugt niemals automatisch eine solche Beziehung und übernimmt keine Dimensionen. Erst die ausdrücklich ausgewählten CiV werden über `INSCRIBES_PURPOSE_IN` mit einem langfristigen Zukunftszustand (`PiF2`) verbunden.
...
| Objekt                               | Bedeutung                                                                                |
| ------------------------------------ | ---------------------------------------------------------------------------------------- |
| `CiV`                                | Beschreibt genau einen dreidimensionalen Wert eines konkreten Werteträgers.              |
| `PiF2`                               | Beschreibt einen langfristigen Zukunftszustand, der aus verbundenen CiV erarbeitet wird. |
| `PiH`                                | Bewahrt einen abgelösten CiV-Zustand oder stellt historischen Kontext bereit.            |
| `RoFOrg`, `RoFTeam`, `RoFTeamMember` | Trägt ein CiV in einem eindeutigen Scope.                                                |
...
PiH ── PROVIDES_CONTEXT_TO ──► CiV ── HELD_BY ──► RoF-Scope
                                      ├── INFORMED_BY ──► anderes CiV
                                      └── INSCRIBES_PURPOSE_IN ──► PiF2
```

### 4.4 Regeln und Ausnahme

1. Jede `CiV`-Instanz beschreibt genau einen Wert; `name` bezeichnet diesen Wert.
2. `notCiV`, `selfCiV` und `toServeCiV` sind nicht leer und beschreiben unterschiedliche, gemeinsam erforderliche Dimensionen desselben Werts.
3. Jedes `CiV` besitzt genau ein `HELD_BY` zu einer `RoFOrg`, einem `RoFTeam` oder einem `RoFTeamMember` mit `memberType = HUMAN`.
4. Derselbe Wertname in verschiedenen Scopes erzeugt eigenständige CiV. Werte und Dimensionen werden zwischen Mensch, Team und Organisation nicht automatisch vererbt oder kopiert.
5. `INFORMED_BY` verbindet ein CiV optional mit keinem, einem oder mehreren anderen CiV, die seine ausdrücklich bestätigte Erarbeitung beeinflusst haben. Selbstbeziehungen sind unzulässig.
6. Die Erzeugung und jede Änderung eines CiV bleibt über `CREATED_BY` beziehungsweise `ChangeEvent` und `REQUESTED_BY` auf ein gültiges `RoleAssignment` zurückführbar. `SYNC` leitet keine menschliche Wertentscheidung ab.
   Für die menschliche Entscheidung ist zusätzlich der konkrete Freigabenachweis nach Abschnitt 12.9 erforderlich; technische Erstellungsprovenienz allein genügt nicht.
7. Ein `CiV` kann über `INSCRIBES_PURPOSE_IN` keinen, einen oder mehrere `PiF2` begründen. Dadurch kann es bereits bestehen, bevor ein Zukunftszustand formuliert wurde.
8. Jedes `PiF2` muss durch mindestens ein `CiV` wertbezogen begründet sein. Alle ein `PiF2` unmittelbar begründenden CiV besitzen denselben `HELD_BY`-Zielknoten; dieser gemeinsame Werteträger bestimmt den Scope des `PiF2`.
9. Historische `PiH` können Kontext für die Bildung oder Änderung eines `CiV` bereitstellen.
10. Wird ein `CiV`, sein Scope oder seine `INFORMED_BY`-Herkunft geändert, muss `SYNC` die Auswirkungen auf alle verbundenen `PiF2` und abhängigen Zukunfts- und Arbeitspfade prüfen. Nur tatsächlich geänderte Zustände erhalten ein eigenes `PiH`.

### 4.5 Beispiel

Die Organisation beschreibt den einzelnen Wert „Klarheit“ und ordnet ihn sich selbst zu:

```text
CiV: Klarheit
  notCiV     = Entscheidungen und Verantwortlichkeiten bleiben nicht absichtlich unklar.
  selfCiV    = Wir machen Zusammenhänge, Zuständigkeiten und Entscheidungen nachvollziehbar.
  toServeCiV = Kunden und Partner erhalten die Orientierung, die sie für gutes Handeln benötigen.
  └── HELD_BY ──► RoFOrg: Beispiel GmbH
```

Gemeinsam mit weiteren organisationsbezogenen CiV wird daraus ein langfristiger Zukunftszustand erarbeitet:

```text
CiV: Klarheit ── INSCRIBES_PURPOSE_IN ──► PiF2

PiF2 = Die Organisation bietet langfristig einen verlässlichen,
       transparenten und kundenorientierten Service.
```

Ändert sich eine Dimension oder der Werteträger, prüft `SYNC` die Auswirkungen auf dieses `PiF2`. Der bisherige CiV-Zustand wird vor der Änderung als `PiH` festgehalten.
...
`PiF2` beschreibt einen langfristigen Zukunftszustand mit einem Zeithorizont von mehr als zehn Jahren bis zu einem offenen Zeitpunkt `n`. Er formuliert, welche langfristige Zukunft aus einem oder mehreren ausdrücklich verbundenen CiV heraus gewollt ist.
...
Ein `PiF2` entsteht, indem die drei Dimensionen eines oder mehrerer CiV desselben Werteträgers gemeinsam in einen langfristigen Zukunftszustand übersetzt werden. Der Zweck wird dabei im `PiF2` erarbeitet und nicht als redundantes Feld am CiV vorformuliert. Strategische Zukunftszustände (`PiF1s`) konkretisieren ihre Beiträge zu diesem langfristigen Zustand.
...
| Objekt  | Bedeutung                                             |
| ------- | ----------------------------------------------------- |
| `CiV`   | Begründet das `PiF2` mit seinen drei Wertdimensionen. |
...
2. Alle unmittelbar begründenden CiV eines `PiF2` besitzen genau denselben Werteträger. Aus diesem gemeinsamen `HELD_BY`-Ziel wird der Scope des `PiF2` abgeleitet.
3. Ein `PiF2` kann Beiträge von keinem, einem oder mehreren `PiF1s` erhalten; `0..n` erlaubt einen noch nicht ausformulierten Entwurf.
4. Ein `PiF1s` muss zu mindestens einem `PiF2` beitragen und kann mehrere `PiF2` unterstützen.
5. `PiF2` beschreibt einen Zustand und keine Tätigkeit.
6. Eine Änderung an `PiF2` führt nicht automatisch zur Änderung aller verbundenen Elemente; `SYNC` prüft zunächst die Auswirkungen.
7. Ein `PiF2` darf `ACHIEVED` nur erreichen, wenn mindestens ein aktuelles `PiF1s` beiträgt. Bei `contributionMode = ALL` müssen alle aktuellen direkten Beiträge `ACHIEVED` sein; bei `ANY` muss mindestens ein aktueller direkter Beitrag `ACHIEVED` sein.
8. Als aktuell gelten beitragende Elemente, die nicht `REPLACED` oder `REVOKED` sind. Ersetzte Beiträge werden über ihren `REPLACED_BY`-Nachfolger berücksichtigt, sobald dieser zum selben Ziel beiträgt.

### 5.5 Beispiel

```text
CiV: Klarheit ── HELD_BY ──► RoFOrg: Beispiel GmbH
  └── INSCRIBES_PURPOSE_IN ──► PiF2
...
`RaN` steht für **Rules and Norms** und beschreibt Regeln, Normen, Grenzen und Leitplanken des JCI-Modells. Ein RaN schützt die ausdrücklich verbundenen Werte (`CiV`) und langfristigen Zukunftszustände (`PiF2`). Es bestimmt über seine geregelten Umsetzungselemente, unter welchen Bedingungen Zukunftskonkretisierungen, Aufgaben, Ergebnisse, Prüfungen, Organisation, Rollen und Umweltinteraktionen zulässig sind.

`RaN` ist kein linearer Schritt der Zukunftskette. Es wirkt als Querschnittselement auf die jeweils geregelten Ziele.

### 6.2 Entstehung

Ein `RaN` entsteht durch die ausdrückliche Formulierung einer fachlichen Regel, rechtlichen Vorgabe, internen Norm oder verbindlichen Grenze. Die Regel benennt die zu schützenden CiV und PiF2, ihre Wirkung, die geregelte Entscheidung, ihren Geltungsbereich, die betroffenen Umsetzungstypen und eine maschinenlesbare Bedingung. `PROTECTS` dokumentiert, was und warum geschützt wird; `GOVERNS` verbindet die konkrete Umsetzung, an der die Regel durchgesetzt wird.

Ein Entwurf darf zunächst ohne Ziel bestehen. Bevor ein `RaN` aktiv wird, muss es mindestens ein CiV und ein PiF2 schützen sowie mindestens ein Umsetzungselement regeln. Wird ein bestehendes `RaN` geändert, prüft `SYNC` alle geschützten und geregelten Ziele und davon abhängigen Elemente. Der abgelöste RaN-Zustand wird als `PiH` festgehalten.
...
| Beziehung  | Bereich                 | Mögliche Ziele                                           |
| ---------- | ----------------------- | -------------------------------------------------------- |
| `PROTECTS` | Wert                    | `CiV`                                                    |
| `PROTECTS` | langfristige Zukunft    | `PiF2`                                                   |
| `GOVERNS`  | Zukunftskonkretisierung | `PiF1s`, `PiF1t`, `PiF1o`                                |
| `GOVERNS`  | Arbeit                  | `Task`                                                   |
| `GOVERNS`  | Erfolg und Prüfung      | `SuccessCriterion`, `Result`, `Verification`, `Evidence` |
| `GOVERNS`  | Organisation            | `RoFOrg`, `RoFTeam`, `RoFTeamMember`                     |
| `GOVERNS`  | Organisationsbeziehung  | `RoFOrgRelationship`                                     |
| `GOVERNS`  | Rollen                  | `RoFRole`, `RoleAssignment`                              |
| `GOVERNS`  | Umwelt                  | `ERoFObject`                                             |

Die Beziehungen werden wie folgt gespeichert:

```text
RaN ── PROTECTS ──► CiV oder PiF2
...
Die Kardinalität `0..n` gilt für gespeicherte Beziehungen und erlaubt unvollständige Entwürfe. Ein aktives `RaN` besitzt mindestens ein `PROTECTS` zu einem CiV, mindestens ein `PROTECTS` zu einem PiF2 und mindestens ein aktuelles Ziel über `GOVERNS`. Jedes geschützte CiV begründet mindestens eines der durch dasselbe RaN geschützten PiF2; jedes geschützte PiF2 wird durch mindestens ein von demselben RaN geschütztes CiV begründet. Das RaN muss nicht alle weiteren CiV des PiF2 schützen. `SYNC` hält die geregelte Zielmenge bei neuen, geänderten oder aus dem Scope ausscheidenden Entitäten aktuell; Schutzbeziehungen werden nur durch menschlich bestätigte Änderungen gesetzt.
...
Der Schutzscope muss organisatorisch mit dem Geltungsbereich vereinbar sein. `GLOBAL` darf Schutzobjekte verschiedener Scopes verbinden. Bei `ORGANIZATION` liegen die Werteträger der geschützten CiV beziehungsweise PiF2 innerhalb derselben Organisation, ihrer Teams oder ihrer menschlichen Mitglieder. Bei `TEAM` dürfen sie das Team, ein gültiges menschliches Teammitglied oder die übergeordnete Organisation bezeichnen. Bei `ENTITY` müssen die geschützten CiV und PiF2 über den WHY-Pfad der direkt geregelten Umsetzungselemente erreichbar sein. Unverbundene fremde Organisationen sind unzulässig.
...
2. Ein RaN-Entwurf darf noch keine vollständigen Schutz- oder Regelziele besitzen.
3. Ein aktives `RaN` schützt mindestens ein `CiV` und mindestens ein `PiF2` und regelt mindestens ein zulässiges Umsetzungselement.
4. Jedes geschützte CiV begründet mindestens eines der durch dasselbe RaN geschützten PiF2 über `INSCRIBES_PURPOSE_IN`.
5. Jedes geschützte PiF2 wird durch mindestens ein CiV begründet, das dasselbe RaN schützt.
...
13. Vereinbare anwendbare Regeln gelten gemeinsam und werden nicht allein aufgrund einer niedrigeren Priorität ignoriert.
14. Widersprechen sich anwendbare Regeln mit unterschiedlicher `priority`, gilt für genau diesen Widerspruch die größere Zahl. Der `ruleType` erzeugt keinen automatischen Vorrang.
15. Widersprechen sich mindestens zwei anwendbare Regeln mit derselben höchsten einschlägigen Priorität, erzeugt `SYNC` genau einen offenen `RaNConflict` für diese Konfliktmenge und Entscheidung.
16. Kann die Vereinbarkeit oder Anwendbarkeit nicht eindeutig bewertet werden, erzeugt `SYNC` einen offenen `RaNConflict` mit `conflictType = UNEVALUABLE` und trifft keine automatische Entscheidung.
17. Ein offener `RaNConflict` besitzt einen für Veränderungsauftrag, Regelmenge und Entscheidung deterministischen `conflictKey`, benennt mindestens eine `CONFLICTING_RULE`, mindestens eine betroffene `JCIEntity` und genau das erkennende `SyncEvent`. Bei `PRIORITY_TIE` sind mindestens zwei Regeln erforderlich; bei `UNEVALUABLE` kann bereits eine nicht eindeutig auswertbare Regel genügen. Derselbe Synchronisationsauftrag darf denselben Konflikt nicht mehrfach anlegen.
18. Ein Konflikt darf nur durch eine ausdrückliche fachliche Änderung und einen nachfolgenden erfolgreichen Synchronisationslauf auf `RESOLVED` gesetzt werden. Er besitzt dann genau ein `RESOLVED_BY` und ein `RESOLVED_THROUGH`.
19. Die Auflösung verändert nicht rückwirkend das protokollierte `SyncEvent`; der Statuswechsel des Konflikts wird wie jede andere fachliche Änderung historisiert.
20. `governedTypes` ist nicht leer. Jedes Ziel von `GOVERNS` besitzt einen darin aufgeführten konkreten `entityType`.
21. `GLOBAL` und `ENTITY` besitzen kein `APPLIES_IN`; `ORGANIZATION` und `TEAM` besitzen genau eine typgerechte `APPLIES_IN`-Beziehung.
22. Eine nicht erfüllte anwendbare `REQUIRE`-Regel oder eine erfüllte `PROHIBIT`-Regel blockiert die Entscheidung als Regelverletzung, auch wenn keine andere Regel widerspricht.
23. Priorität wird ausschließlich bei gleichzeitigem Erlauben und Verweigern derselben Entscheidung verwendet. Zwei verweigernde oder zwei erlaubende Regeln sind kein Prioritätskonflikt.
24. `PROTECTS` ersetzt weder `GOVERNS` noch `APPLIES_IN`; alle drei Beziehungen erfüllen getrennte, gemeinsam erforderliche Funktionen.
...
4. Jeder `PiF1o` wird in mindestens einen `Task` zerlegt; jeder Task bleibt unabhängig von Hierarchiestufe und aktuellem Status direkt genau einem `PiF1o` zugeordnet. Die Abschlussauswertung verwendet den aktuellen Umfang nach Abschnitt 9.4.2.
...
7. Ein `COMPOSITE`-Task besitzt mindestens einen direkten Untertask und keine Beziehungen `EXECUTED_BY`, `USES` oder `PRODUCES`. Für aktuelle Composites ist mindestens ein aktueller direkter Untertask erforderlich. Eigene Voraussetzungen haben vor der Statusaggregation Vorrang.
...
11. Accountability für den `PiF1o`, Verantwortung des Teams für den Task und tatsächliche Ausführung atomarer Tasks durch `RoleAssignments` bleiben fachlich getrennt.
    Die Freigabe wird als weitere getrennte Entscheidung nach Abschnitt 12.9 geprüft. Eine Genehmigung ersetzt weder Ausführungsberechtigung noch Abschlussbestätigung.
...
13. `DEPENDS_ON` darf zwischen Tasks desselben oder verschiedener `PiF1o` bestehen, aber weder Selbstbezüge noch Zyklen bilden. Zusätzlich muss die gemeinsame Abschlussprüfsicht aus Hierarchie und Voraussetzungen nach Abschnitt 9.4.2 zyklusfrei sein. Eine Voraussetzung ist nur mit `status = COMPLETED` erfüllt.
14. Ein freigegebener Task in `ACTIVE` oder `BLOCKED` mit mindestens einer unerfüllten eigenen Voraussetzung wird `BLOCKED`; ein Abschluss ist dann unzulässig. `DRAFT` benötigt zuerst eine ausdrückliche Freigabe, terminale Tasks werden nicht umgeschrieben. `REPLACED` und `REVOKED` erfüllen keine Voraussetzung; eine gültige Nachfolge muss ausdrücklich neu über `DEPENDS_ON` verbunden werden.
...
16. Die Ableitung eines freigegebenen Composite-Status folgt ausschließlich der geordneten Tabelle in Abschnitt 9.4.2: eigene unerfüllte Voraussetzung vor Untertask-Aggregation; nur aktuelle direkte Untertasks; kein automatischer Rückfall auf `DRAFT`. Ersatz, Widerruf und nicht leere aktuelle Hierarchie werden ausdrücklich geprüft.
...
18. Eine `Verification` bewertet genau ein `COMPLETED`-Result und prüft genau ein `ACTIVE`-SuccessCriterion. Bei ihrer Erzeugung und für die aktuelle Zielauswertung gehören der erzeugende Task und das Kriterium zum aktuellen Umfang desselben `PiF1o`.
19. Bei der Erzeugung entsprechen `evaluatedResultRevision` und `checkedCriterionRevision` exakt den unter dem Commit-Gate nach Abschnitt 12.8 geprüften Zielrevisionen. Jede Abweichung erzeugt `CONFLICT`. `EVALUATES`, `CHECKS` und eigene Prüfungsprovenienz verändern diese Zielrevisionen nach Abschnitt 2.2.8 nicht.
...
24. Eine anwendbare aktuelle `Verification` ist `COMPLETED`, nicht über `SUPERSEDES` abgelöst und bindet weiterhin die aktuellen Revisionen ihres `COMPLETED`-Results und `ACTIVE`-Kriteriums. Für die Zielauswertung müssen erzeugender Task und Kriterium zusätzlich zum aktuellen Umfang desselben `PiF1o` gehören.
...
29. Jedes fachlich vollständige `PiF1o` besitzt mindestens ein aktuelles `SuccessCriterion` mit `requirementLevel = REQUIRED`. Ausgeschiedene Pflichtkriterien erfüllen diese Mindestmenge nicht.
30. Ein `PiF1o` darf `ACHIEVED` nur erreichen, wenn seine aktuelle Taskmenge nicht leer und vollständig `COMPLETED` ist, mindestens ein aktuelles `REQUIRED`-Kriterium besteht, jedes aktuelle Pflichtkriterium `ACTIVE` und durch anwendbare aktuelle Verifications erfüllt ist und alle relevanten Abhängigkeits-, WHY-, WHO-, Modell- und `RaN`-Bedingungen eingehalten sind.
...
Die Kriterienbedingung verwendet ausschließlich `K_current(P)` nach Abschnitt 9.4.2. Mindestens ein aktuelles `REQUIRED`-Kriterium muss bestehen und jedes muss `ACTIVE` und erfüllt sein. `OPTIONAL`-Kriterien werden ohne Blockade von `ACHIEVED` ausgewertet und dokumentiert. Berücksichtigt werden nur anwendbare aktuelle Verifications mit Results aus `T_current(P)`; zusätzlich müssen alle Task- und Modellbedingungen erfüllt sein.
...
```text
PiF1o.ACHIEVED
= T_current(P) ist nicht leer und jedes Mitglied ist COMPLETED
+ mindestens ein aktuelles REQUIRED-Kriterium besteht
+ jedes aktuelle REQUIRED-Kriterium ist ACTIVE und erfüllt
+ Abhängigkeiten und anwendbare Prüfrevisionen sind gültig
+ alle relevanten WHY-, WHO-, Modell- und RaN-Bedingungen gelten
```

#### 9.4.2 Aktueller Arbeitsumfang und gemeinsame Abschlussauswertung

Die gespeicherte Zuordnung beschreibt Herkunft und Zielkontext. Sie bleibt auch für `REPLACED` und `REVOKED` erhalten. Für die aktuelle Auswertung gelten daraus abgeleitete Mengen:

```text
T_all(P)     = alle über PiF1o DECOMPOSES_INTO direkt zugeordneten Tasks
T_current(P) = T_all(P) ohne Status REPLACED oder REVOKED
K_all(P)     = alle über HAS_SUCCESS_CRITERIA zugeordneten Kriterien
K_current(P) = K_all(P) ohne Status REPLACED oder REVOKED
children_current(C) = direkte Untertasks von C ohne REPLACED oder REVOKED
```

`COMPLETED` bleibt aktuell und zählt weiter zum Abschlussnachweis. `ACHIEVED` verlangt mindestens einen aktuellen Task, dessen gesamte aktuelle Taskmenge `COMPLETED` ist, sowie mindestens ein aktuelles `REQUIRED`-Kriterium. Jedes aktuelle Pflichtkriterium muss `ACTIVE` und erfüllt sein. Ein leeres Ergebnis nach Filterung ist kein Erfolg. Eine Verification zählt nur, wenn ihr erzeugender Task aktuell demselben `PiF1o` zugeordnet ist. Ergebnisse ausgeschiedener Tasks bleiben erhalten, erfüllen den aktuellen Umfang aber nicht automatisch. Eine fachlich gewünschte Übernahme benötigt einen aktuellen Task mit eigenem Result und eine neue Prüfung; das frühere Result kann als nachvollziehbarer Nachweis dienen, seine ursprüngliche `PRODUCES`-Zuordnung bleibt erhalten.

Widerruf, Ersatz und eine Veränderung des Pflichtumfangs benötigen einen ausdrücklich autorisierten Veränderungsauftrag. `SYNC` entfernt keine Verpflichtung, nur um `ACHIEVED` zu ermöglichen. Ein Ersatz zählt erst nach ausdrücklicher Zuordnung zum selben `PiF1o` und gegebenenfalls zum selben aktuellen Parent; die frühere Zuordnung bleibt bestehen. Abhängigkeiten auf einen ausgeschiedenen Task bleiben unerfüllt, bis sie ausdrücklich geändert werden. Ein aktueller Composite besitzt mindestens einen aktuellen direkten Untertask. Ein ausgeschiedener Composite darf aktuelle Nachfahren nicht aus der Auswertung verschwinden lassen: Sie bleiben über ihre direkte Zielzuordnung aktuell und benötigen eine konsistente aktuelle Hierarchie; andernfalls entsteht `CONFLICT`. Kein automatischer Widerruf des Teilbaums. Nach `ACHIEVED` wird ein geänderter Umfang durch ein neues Ziel modelliert.

Für einen bereits freigegebenen `COMPOSITE` in `ACTIVE` oder `BLOCKED` gilt folgende Reihenfolge:

| Prüfung in dieser Reihenfolge                                             | Ergebnis    |
| ------------------------------------------------------------------------- | ----------- |
| mindestens eine eigene unerfüllte `DEPENDS_ON`-Voraussetzung              | `BLOCKED`   |
| alle aktuellen direkten Untertasks `COMPLETED`                            | `COMPLETED` |
| mindestens ein aktueller direkter Untertask `ACTIVE`                      | `ACTIVE`    |
| kein `ACTIVE`, aber mindestens ein aktueller direkter Untertask `BLOCKED` | `BLOCKED`   |
| nur `DRAFT` oder eine Mischung aus `DRAFT` und `COMPLETED`                | `ACTIVE`    |

Ein `DRAFT` wird ausschließlich durch die ausdrückliche reguläre Freigabe mit vollständigen Aktivierungsprüfungen `ACTIVE` oder `BLOCKED`. Bei ausdrücklicher Composite-Freigabe bestimmt dieselbe Priorität eigener Voraussetzungen und blockierter Kinder den Status `ACTIVE` oder `BLOCKED`; selbst bei vollständig abgeschlossenen Kindern entsteht zunächst `ACTIVE`. Der Abschluss benötigt eine spätere Auswertung dieses freigegebenen Zustands. Eine Umfangsänderung ersetzt die Freigabe nicht und bewirkt niemals direkt `DRAFT → COMPLETED`. Ein freigegebener Composite fällt bei ausschließlich entworfenen Untertasks nicht auf `DRAFT` zurück. Terminale Tasks werden durch Ableitung nicht wieder geöffnet. Eine eigene Composite-Voraussetzung sperrt dessen Abschluss; sie wird nicht stillschweigend auf Untertasks vererbt. Soll bereits deren Ausführung warten, müssen die betroffenen Tasks eigene ausdrückliche Voraussetzungen besitzen.

Für die Abschlussreihenfolge wird ein virtueller Graph aus beiden gespeicherten Kantentypen gebildet. Er ist eine Prüfsicht, kein neuer Beziehungstyp:

```text
waits(C, child) für jeden aktuellen direkten Untertask eines aktuellen Composite C
waits(T, prerequisite) für jede ausdrückliche DEPENDS_ON-Voraussetzung eines aktuellen T
Kantenrichtung: Quelle wartet auf Ziel.
Zyklusprüfung: gemeinsamer Kandidatengraph über alle erreichbaren PiF1o.
Auswertung: Voraussetzungen zuerst, dann wartende Tasks; ATOMIC und COMPOSITE gemeinsam.
```

Die bisherigen Prüfungen der gespeicherten Hierarchie, der Abhängigkeiten und der Ersatzketten bleiben bestehen. Zusätzlich führt jeder Zyklus in der gemeinsamen Abschlussprüfsicht zu `CONFLICT` mit vollständigem Zykluspfad. Beispiel: `C DECOMPOSES_INTO A` und `A DEPENDS_ON C` ergeben `C → A → C`, obwohl beide Kantentypen einzeln zyklusfrei sind. Ein ausgeschiedener Voraussetzungstask ist ein unerfülltes Ziel und wird nicht als erfüllte oder fehlende Bedingung ausgeblendet. Alle Operationen eines Auftrags werden gemeinsam auf dem Kandidatengraphen geprüft; die Commit-Sperre nach Abschnitt 12.8 verhindert, dass konkurrierende Änderungen zusammen einen ungeprüften Zyklus erzeugen.
...
- Welche eigenen Werte werden von einer Organisation, einem Team oder einem Menschen getragen?
...
Ein organisationsbezogenes CiV wird über `HELD_BY` der `RoFOrg` zugeordnet, auch wenn ein Geschäftsführungs- oder anderes beauftragtes Team den Wert erarbeitet. Ein teambezogenes CiV verweist auf das betreffende `RoFTeam`, ein persönliches CiV auf ein `RoFTeamMember` mit `memberType = HUMAN`. Der Werteträger und die an der Erarbeitung handelnden Rollen bleiben damit getrennt: `HELD_BY` beschreibt, wessen Wert es ist; `CREATED_BY` und `REQUESTED_BY` beschreiben, wer ihn im Modell angelegt oder geändert hat.
...
| `CiV`                | Einzelner Wert, der von Organisation, Team oder Mensch getragen wird. |
...
21. Das Team jedes vertretenden `RoleAssignment` muss zur jeweils vertretenen `RoFOrg` gehören.
22. Eine beteiligte Organisation wird nicht zusätzlich als `ERoFObject` dupliziert.
23. Jeder `PiF1o` besitzt genau ein accountable `RoFTeamMember`. Jeder seiner Tasks besitzt genau ein verantwortliches `RoFTeam`; nur aktive oder abgeschlossene atomare Tasks benötigen mindestens ein ausführendes `RoleAssignment`. Das accountable Mitglied muss den Task nicht selbst ausführen.
    Auch `PiF2`, `PiF1s` und `PiF1t` dürfen jeweils genau ein accountable Mitglied benennen. Die Speicherkardinalität `0..1` erlaubt den schrittweisen Aufbau; eine tatsächlich benötigte Freigabeebene darf ohne eindeutige Accountability nicht übersprungen werden. Das Mitglied wird nicht automatisch von einer anderen Ebene geerbt.
24. Ein `HELD_BY` zu einem `RoFTeamMember` ist nur zulässig, wenn `memberType = HUMAN` gilt. Technische Mitglieder besitzen kein persönliches Wertkonstrukt.
25. Der Scope eines CiV wird ausschließlich durch `HELD_BY` bestimmt. Teammitgliedschaften oder Organisationszugehörigkeiten erzeugen keine zusätzlichen impliziten Scopes.
26. Ein organisationsbezogenes CiV bleibt der `RoFOrg` zugeordnet, auch wenn die Erarbeitung durch ein Geschäftsführungs- oder anderes beauftragtes Team erfolgt. Die handelnden Personen bleiben über ihre `RoleAssignments` nachvollziehbar.
...
Für freigabepflichtige Vorgänge enthält die `SyncDefinition` zusätzlich `approvalProfileVersion = "1.0"`, unterstützt `APPROVED_BY` und bindet über ihre Paketprüfsumme den Freigabevalidator. Unbekanntes oder fehlendes Profil bedeutet keine Ausführungsbefugnis.
...
**Kurzes Beispiel:** `SYNC 2.0` verweist auf Ontologie 2.0, Graphregeln 2.0 und das Handler-Paket `jci-sync-2.0` mit dessen Prüfsumme. Ein `SyncEvent` hält dadurch nicht nur „SYNC 2.0“, sondern die exakt ausgeführte Definition nachvollziehbar fest.

### 12.2 Entstehung

Ein Synchronisationsversuch entsteht aus einem angenommenen Veränderungsauftrag:

1. Vor Ausführung erfasst die Annahme den vollständigen normalisierten Auftrag, sein unveränderliches `ChangeEvent` und die wiederaufnehmbare Laufeinplanung nach Abschnitt 12.8 dauerhaft und atomar. Das gilt für normale Änderungen, `CREATED` und `HISTORICAL_CORRECTION`.
...
4. Nach Erwerb des bestandsweiten technischen Gate liest die `SYNC`-Definition alle direkt und indirekt relevanten Elemente, Beziehungen und erforderlichen Abwesenheiten nach Abschnitt 12.8 frisch.
...
8. Bei Task-Änderungen bildet sie die gemeinsame Abschlussprüfsicht aus Hierarchie und Voraussetzungen, prüft den vollständigen Kandidatengraphen und wertet atomare und zusammengesetzte Tasks gemeinsam mit Voraussetzungen zuerst aus.
9. Bei einer abgeschlossenen `Verification` prüft sie gebundene Revisionen, aktuellen Task- und Kriterienumfang sowie alle aktuellen `REQUIRED`-Kriterien des zugehörigen `PiF1o`.
10. Unmittelbar vor Übernahme prüft sie die vollständige relevante Graphsicht, Run-Eigentümerschaft, gebundene Revisionen und zeitabhängigen Vorbedingungen am geschützten fachlichen Entscheidungspunkt.
...
12. Bei `SUCCESS` werden das tatsächliche Fachdelta, zulässige Folgeänderungen, zugehörige `PiH`, Korrekturen, Konfliktauflösungen, Beziehungen, erfolgreiches `SyncEvent` und idempotenter Commitbeleg in einer atomaren Transaktion gespeichert.
...
Bei `SUCCESS` sind Fachänderungen, Abschlussdokumentation und idempotenter Commitbeleg zwingend atomar. Bei `CONFLICT` oder `FAILED` wird kein unzulässiges Fachdelta übernommen. Verhindert ein Datenbankabbruch die Abschlussdokumentation, wird diese in einer neuen geschützten Transaktion mit derselben `runId` nachgeholt; unklarer Erfolg ist vor jeder Wiederholung anhand des dauerhaften Commitbelegs zu klären. Abschnitt 12.8 definiert den vollständigen Wiederaufnahmevertrag.
...
CiV, ihre Werteträger und `INFORMED_BY`-Herkünfte sowie PiF2 bis PiF1o
...
9. Bei `CREATED` müssen Ziel-ID frei und `requestedRevision = null` sein. Nur `SUCCESS` erzeugt das neue Ziel mit `revision = 1`, `CREATED_BY` und genau einem `CHANGED_BY`. Für dieses neue Ziel entsteht kein `PiH`; vorhandene veränderliche Eigentümer, deren fachliche Beziehungen sich dabei ändern, erhalten nach Abschnitt 2.2.8 ein eigenes `PiH`. `CONFLICT` oder `FAILED` übernimmt weder das Ziel noch ein Fachdelta oder Historie.
...
17. `SYNC` serialisiert alle JCI-Schreibwege nach Abschnitt 12.8 und berechnet die aktuelle `HistoryView` unter dem Gate erneut. Ein abweichender `expectedHistoryViewHash`, ein Profilfehler oder eine Pfadüberlappung außerhalb der Ablöseregel erzeugt `CONFLICT` ohne Korrektur.
18. Nicht abgelöste Korrekturen eines `PiH` besitzen untereinander und jeweils intern überlappungsfreie kanonische Pfade nach Abschnitt 2.2.9. Gleichheit sowie Vorfahr-/Nachfahrbeziehungen zählen als Überlappung. Keine Überlappung erlaubt kein `SUPERSEDES`. Genau eine überlappte aktive Korrektur muss vollständig abgelöst werden. Mehr als eine überlappte aktive Korrektur erzeugt wegen `SUPERSEDES = 0..1` einen `CONFLICT`.
...
24. Eine neue `Verification` bindet genau die geprüften Zielrevisionen. Unter dem Gate prüft `SYNC` den vollständigen relevanten Zustand und die abschließende Zeitgültigkeit; das Anlegen der Prüfbezüge verändert die gebundenen Revisionen selbst nicht.
...
27. Ein `PiF1o` darf `ACHIEVED` nur erreichen, wenn seine aktuelle Taskmenge nicht leer und vollständig `COMPLETED` ist, mindestens ein aktuelles `REQUIRED`-Kriterium besteht, jedes aktuelle Pflichtkriterium `ACTIVE` und durch anwendbare aktuelle Verifications erfüllt ist und alle relevanten Abhängigkeits-, WHY-, WHO-, Modell- und `RaN`-Bedingungen eingehalten sind. Optionale Kriterien blockieren den Übergang nicht.
...
29. Bei Task-Änderungen prüft `SYNC` Typ, aktuellen Umfang, Parents, Subtasks, ausdrückliche Voraussetzungen, abhängige Tasks und betroffene `PiF1o`. Die gemeinsame Abschlussprüfsicht muss zyklusfrei sein; atomare und zusammengesetzte Status werden gemeinsam mit Voraussetzungen zuerst abgeleitet und nur bei tatsächlichem fachlichem Delta historisiert.
...
31. Bei Änderungen an CiV, `HELD_BY` oder `INFORMED_BY` prüft `SYNC` die drei Pflichtdimensionen, den zulässigen Werteträger, die menschliche Entscheidungsprovenienz und den gemeinsamen Scope aller dadurch begründeten `PiF2`. Es erzeugt oder übernimmt keine Wertentscheidung automatisch.
...
PiF1o: Antwort innerhalb von 48 Stunden
                  ...
                  └── CHANGED_BY ──► ChangeEvent
                                          ...
                                          └── startet ──► SyncRun (technisch)
                                                               │
                                                               └── Abschluss ──► SyncEvent
                                                                                     ├── EXECUTES ──► SYNC: JCI-Standardprozess 2.0
                                                                                     ├── AFFECTS ──► PiF1o
...
Der technische `SyncRun` verwendet die SYNC-Definition `JCI-Standardprozess 2.0`. Diese prüft den `PiF1o`, seine Erfolgskriterien, Tasks, Verantwortung, Rollenaktivierungen und verwendeten `ERoFObjects`. Der bisherige PiF1o-Zustand wird als `PiH` festgehalten. Abhängige Elemente erhalten nur dann ein eigenes `PiH`, wenn ihr Zustand tatsächlich geändert wird. Das unveränderliche `SyncEvent` wird zusammen mit dem Fachdelta und Commitbeleg atomar gespeichert; es dokumentiert die verwendete Definition sowie alle Prüfungen, Änderungen und Konflikte.

---

### 12.6 Technologieunabhängiges Austauschformat

JCI verwendet UTF-8-kodierte JSON-Dokumente für Änderungsaufträge und SYNC-Ergebnisse. Die hier beschriebene, gegenüber Version 1.1 inkompatibel präzisierte Form besitzt `schemaVersion = "2.0"`. UUIDs werden als Zeichenketten, Zeitpunkte nach ISO 8601 mit Zeitzone und fachliche Werte als `TypedValue` übertragen.
...
`APPROVED_BY` ist davon ausdrücklich ausgenommen: Diese Auditbeziehung darf nicht durch generische Operationen angefordert werden. Sie entsteht ausschließlich aus vollständig geprüften Freigabebelegen bei der Annahme nach Abschnitt 12.9.
...
Die verbindlichen maschinenlesbaren Schemas sind [`jci-change-request.schema.json`](schemas/jci-change-request.schema.json), [`jci-sync-result.schema.json`](schemas/jci-sync-result.schema.json) und [`jci-history-snapshot.schema.json`](schemas/jci-history-snapshot.schema.json). Das Snapshotschema beschreibt den Inhaltspayload `{stateData, relationshipData}`; die Profilwahl bleibt bei `PiH.snapshotSchemaVersion`. Frühere Transportschemas bleiben unter [Legacy 1.1](schemas/legacy/1.1/) unverändert erhalten. Strukturelle Schemaprüfung ersetzt keine Prüfung des Kandidatengraphen, der Pfadüberlappung oder des geschützten Commit.
...
### 12.8 Geschützter Abschluss und dauerhafte Wiederaufnahme

Alle JCI-Schreibwege eines Datenbank-/Modellbestands verwenden denselben technischen Commit-Gate. Er gehört nicht zum JCI-Entitäts- oder Beziehungskatalog und gilt unabhängig von Organisationsgrenzen und momentan verbundenen Teilgraphen. Dazu gehören Auftragsannahme, Fachänderungen, neue Verifications, historische Korrekturen, Konflikt- und Abschlussdokumentation, Import und Migration. Bootstrap prüft den leeren `JCIEntity`-Bestand unter derselben eindeutigen Sperre; ein vorab eingerichteter technischer Gate ist keine fachliche Bootstrap-Entität. Ein exklusiver Wartungsmodus darf den Gate nur ersetzen, wenn nachweislich kein anderer Schreibweg aktiv ist.

Die Auftragsannahme speichert den vollständigen normalisierten Payload dauerhaft und bindet ihn zusammen mit `requestId`, `idempotencyKey`, Ziel, `requestedRevision` und Profilversionen unveränderlich. Dieselbe Kennung mit anderem Payload wird abgewiesen. Ein flüchtiger Handleraufruf oder ein Hash ohne wiederherstellbaren Payload genügt nicht. Technische Request-, Run-, Outbox- und Commitbelege sind Betriebszustand außerhalb von `JCIEntity`. Ein Run erhält eine eindeutige Eigentümerschaft mit Fencing; ein abgelöster Worker darf weder Änderungen noch einen widersprechenden Abschluss speichern.

Der verbindliche Ablauf in einer expliziten Transaktion lautet:

1. Den eindeutigen vorhandenen Gate sperren, bevor fachliche Entscheidungsdaten gelesen werden.
2. Dauerhaften Auftrag, Run-Eigentümerschaft und bereits vorhandenen Commitbeleg prüfen. Ein belegter Erfolg wird zurückgegeben, ohne die Fachänderung erneut auszuführen.
3. Unter der Sperre Zielrevisionen und die gesamte relevante Graphsicht frisch lesen: WHY-/WHO-Pfade, gültige Rollen und Mitgliedschaften, Kapazität, alle einschlägigen Regeln, aktuelle Nachweise, Scope, Korrekturhashes, Hierarchie, Voraussetzungen sowie erforderliche Abwesenheiten und Eindeutigkeiten. Nur die Revisionen der Schreibmenge zu prüfen genügt nicht.
4. Alle angeforderten Operationen zu einem Kandidatengraphen zusammenführen; aktuelle Mengen, Status und Regelwirkungen gemeinsam ableiten. Revisionen und `PiH` nach der Eigentümermatrix für das tatsächliche Delta planen. Ein No-op erzeugt keinen fiktiven Vorgängerzustand.
5. Unmittelbar vor Übernahme einen Datenbankzeitpunkt als fachlichen Entscheidungspunkt bestimmen und alle zeitabhängigen Vorbedingungen sowie davon abhängige Ableitungen daran validieren. Dieser Zeitpunkt liegt innerhalb der gehaltenen Sperre und wird im technischen Commitbeleg dokumentiert. Die Zusage gilt für diesen Entscheidungspunkt; die spätere physische Commit-Bestätigung ist kein zweiter fachlicher Bewertungszeitpunkt.
6. Fachdelta, zugehörige `PiH`, zulässige Korrekturen und Konfliktauflösungen, Beziehungen, genau ein erfolgreiches `SyncEvent` und den idempotenten Commitbeleg gemeinsam atomar speichern. Erst danach darf der Erfolg bestätigt werden.

Jeder übernommene JCI-Schreibvorgang erhöht die technische `graphEpoch`, auch wenn nur ein revisionsneutraler Nachweis entsteht. So werden neue Regeln, Kanten und Verifications erkennbar, die eine reine Prüfung vorhandener Entity-Revisionen übersieht. Eine außerhalb der Sperre vorberechnete Variante ist nur mit einer konsistent gelesenen Epoche und Prüfung auf exakt dieselbe Epoche unter dem Gate zulässig; Abweichung bedeutet erneute vollständige Auswertung oder `CONFLICT`. Die Zeitprüfung bleibt erforderlich. Technische Heartbeats dürfen ausgenommen sein, solange sie keine fachlichen Entscheidungsdaten verändern; die Eigentümerschaft wird trotzdem unter dem Gate geprüft.

Bei `CONFLICT` oder `FAILED` werden keine unzulässigen Fachänderungen übernommen. Hat die Datenbank die Transaktion abgebrochen, wird die Abschlussdokumentation in einer neuen geschützten Transaktion mit derselben `runId` nachgeholt. Bei unbekanntem Commit-Ausgang wird zuerst anhand des dauerhaften Belegs geklärt, ob genau dieser Run bereits erfolgreich war; ein Timeout allein rechtfertigt keine zweite Ausführung. Unveränderte Wiederholungen behalten die angeforderte Revision. Eine inzwischen andere gewünschte Ausgangsrevision oder ein anderer Payload benötigen einen neuen Auftrag. Ein dokumentierter No-op darf `SUCCESS` mit `changedCount = 0` und ohne `PiH` liefern.

Die Neo4j-Abbildung und ihr expliziter Transaktionsablauf stehen in [`implementations/neo4j/JCI_NEO4J_SCHEMA.md`](implementations/neo4j/JCI_NEO4J_SCHEMA.md). Die ausführbaren Referenzprüfungen bilden die Entscheidungsregeln und konkurrierende Kandidaten ab; sie sind weder eine produktive SYNC-Engine noch ein Nachweis realer Neo4j-Transaktionsisolation.

### 12.9 Nachweisbare menschliche Freigabe – Profil 1.0

#### 12.9.1 Geltung und getrennte Verantwortung

Das Freigabeprofil ergänzt das Regelpaket 2.0, ohne ein weiteres Kernelement oder einen neuen Graphobjekttyp einzuführen. Eine wartende Freigabeanforderung ist ein dauerhaft gespeicherter technischer Vorschlag, noch kein angenommenes `ChangeEvent`. Menschliche Antworten werden als unveränderliche technische Entscheidungsbelege gespeichert. Auch Ablehnungen und abgebrochene Vorschläge bleiben nachvollziehbar; sie verändern den Task nicht. Erst ein vollständig genehmigter Auftrag wird regulär angenommen und plant atomar seinen ersten `SyncRun` ein. Ein bereits angenommenes Ereignis wartet somit nicht auf nachträglich anzuhängende Genehmigungen.

Freigabepflichtig sind die ausdrückliche erstmalige Task-Freigabe (`Task.action.RELEASE`) und menschliche Modellentscheidungen (`Model.action.CONFIRM`): CiV-Inhalt und Werteträger, `INFORMED_BY`, die wertbezogene PiF2-Bildung über `INSCRIBES_PURPOSE_IN`, `PROTECTS` sowie Änderungen an Freigabepolicies und ihren Verantwortungsgrundlagen. Die Prüfung berücksichtigt beide Kantenrichtungen, Entfernen, Ersatz und Widerruf sowie den alten und neuen Zustand. Eine reine Umbenennung des Auftrags oder das Weglassen des Freigabe-Envelopes darf diese Pflicht nicht umgehen. Nicht unterstützte Operationskombinationen werden abgewiesen, nicht als genehmigungsfrei behandelt. Die technische Vorbereitung eines Vorschlags ist keine bestätigte Wertentscheidung.

`ACCOUNTABLE_MEMBER` benennt einen Ansprechpartner, keine automatische Entscheidungsbefugnis. Ein `PiF1o` behält genau ein accountable Mitglied; `PiF1t`, `PiF1s` und `PiF2` dürfen jeweils höchstens eines besitzen. Eine für die Route benötigte Ebene muss eindeutig besetzt sein. Ein menschlicher Genehmiger handelt über sein gültiges `RoleAssignment`; dessen Mitglied muss über `HAS_ASSIGNMENT` eindeutig mit `memberType = HUMAN` auflösbar sein. Team, Organisation, Rolle, Mitglied und Rollenaktivierung müssen gültig sein; die Zeitintervalle von `HAS_MEMBER`, `HAS_ROLE` und `RoleAssignment` müssen sowohl bei Bestätigung als auch am geschützten Entscheidungspunkt passen. Menschliche Identität darf nicht aus einem vom Antragsteller gelieferten Boolean abgeleitet werden.

Antragsteller, Genehmiger, accountable Mitglied, verantwortliches Team und ausführende Rolle bleiben unterscheidbar. Dieselbe Person darf mehrere Funktionen besitzen; ein generelles Vieraugenprinzip wird nicht erfunden. Falls eine RaN getrennte Personen verlangt, ist auch diese Bedingung verbindlich. Eine Freigabe verleiht keine technischen Zugriffsrechte auf ein `ERoFObject`.

Zeitliche Gültigkeit wird im Freigabeprofil halb offen geprüft: `validFrom <= Prüfzeitpunkt < validUntil`; ohne `validUntil` besteht keine obere Grenze. Der gesamte Zeitraum einer Rollenaktivierung liegt weiterhin innerhalb ihrer Mitgliedschaft und ihres Rollenbesitzes. Der ursprüngliche Antragsteller darf technisch sein, muss aber authentifiziert und über eine gültige Rollenaktivierung nachgewiesen sein.

#### 12.9.2 Ausdrückliche Befugnis und Eskalation

Eine `ApprovalPolicy` ist ein strukturiertes optionales Feld eines `RaN`, keine zusätzliche Kante und kein Akteur. `roleIds` sind exakte Rollen-IDs als Regeloperanden; Namen sind keine Identitäten. Das Format lautet:
...
approvalPolicy = {
  ...
  levels: bei ACCOUNTABLE_CHAIN nicht leere, eindeutige Liste
          aus PiF1o, PiF1t, PiF1s, PiF2; bei VALUE_SCOPE nicht gesetzt oder leer
}
```

Die Befugnis benötigt eine aktive, zeitlich gültige, scope-passende erlaubende `PERMIT`-RaN mit passender Policy, `decisionKey`, Rolle und erfüllter Bedingung. Für Task-Freigaben regelt sie den konkreten Task; für Wertentscheidungen regelt sie die genehmigende Rollenaktivierung im passenden Werteträger-Scope. `GOVERNS` erhält deshalb kein neues Ziel `CiV` oder `PiF2`. Alle einschlägigen Verweigerungen und Konflikte bleiben zu prüfen; bloße Abwesenheit einer verweigernden Regel ist keine Freigabebefugnis. Eine im beantragten Kandidaten neu eingeführte Regel darf nicht ihre eigene Einführung legitimieren: Maßgeblich sind die vorher bereits gültigen Befugnisse.

Die Freigaberoute wird aus dem vollständigen aktuellen Graphen berechnet, nicht vom Antragsteller gewählt:
...
`CONTRIBUTES_TO` bleibt fachlicher Zukunftsbeitrag. Die Route verwendet die Beziehung zur Navigation, macht daraus aber keine Vorgesetzten- oder Berechtigungskante. Bei Wertentscheidungen (`VALUE_SCOPE`) müssen alle betroffenen alten und neuen Werteträger durch passende menschliche Genehmigungen abgedeckt sein; die Task-Zukunftskette ersetzt diese Scope-Prüfung nicht.

Eine `VALUE_SCOPE`-Policy verleiht Befugnis ausschließlich für die Werteträger ihrer geschützten CiV und PiF2. Zusätzlich gelten der organisatorische Akteurskontext und `APPLIES_IN`. Auch `GLOBAL` macht aus dem Schutz fremder Werte keine Freigabebefugnis für einen anderen Werteträger. Ansonsten anwendbare Verweigerungen werden durch diese Begrenzung der erlaubenden Befugnis nicht ausgeblendet.

#### 12.9.3 Begrenzter ausführbarer Bedingungsvertrag
...
Das [Freigabe-Envelope](schemas/jci-approval-envelope.schema.json) enthält `approvalProfileVersion`, `decisionKey`, den unveränderten Basisauftrag als `proposal`, `requestHash`, `contextHash` und `receipts`. `requestHash` ist SHA-256 über UTF-8 des vollständigen kanonischen Basisauftrags nach Profil 2.0; die Belege liegen außerhalb dieses Hashes, um eine zirkuläre Bindung zu vermeiden. Enthalten sind insbesondere Request-ID, Idempotenzkennung, ursprünglicher Antragsteller, Ziel, erwartete Revision, Zeitpunkt, Begründung und sämtliche Operationen.

`contextHash` bindet zusätzlich Freigabeprofil, Entscheidung, Request-Hash, die kanonisch geordnete relevante Fachgraphsicht, die daraus abgeleiteten Anforderungen sowie konfigurierte Anfangsbefugnisse und deren dauerhafte Sperrmerker. Die Sicht enthält Verantwortlichkeiten, Zukunftszweige, RaN und Rollen-/Scope-Grundlagen einschließlich ihrer Revisionen und Beziehungseigenschaften. Reine Audit- und Prozessbezüge werden ausgeschlossen, damit das Speichern eines Belegs nicht seine eigene Fachgrundlage verändert. Fehlt der vollständige nachprüfbare Kontext, ist der Hash allein kein Nachweis.

Jeder Beleg besitzt `receiptId`, `requestHash`, `contextHash`, `decidedAt`, `validUntil`, `outcome = APPROVED | REJECTED`, `roleAssignmentId`, `memberId` und `attestation`. Es gilt `decidedAt <= decisionAt < validUntil`. Eine vertrauenswürdige Authentifizierungsintegration muss bestätigen, dass der nachgewiesene Mensch genau diesen Inhalt bestätigt hat; eine beliebige Attestationszeichenkette oder selbst behauptete Member-ID genügt nicht. Die Referenzprüfung verlangt dafür einen ausdrücklichen Prüfadapter und gibt ohne verifizierten Nachweis keine Freigabe. Produktionsbetrieb muss Identitätsbindung, manipulationsgeschützte Speicherung und Schlüssel-/Sitzungsprüfung selbst bereitstellen.

Ein Envelope enthält höchstens einen Beleg je Rollenaktivierung. `approvalHash` ist SHA-256 über den vollständigen kanonischen Beleg. Beim Annehmen werden alle genehmigenden Rollen über `APPROVED_BY` mit dem neuen `ChangeEvent` verbunden und `receiptId`, `decidedAt`, `requestHash` und `approvalHash` an dieser Kante gespeichert. Die vollständigen unveränderlichen Belege und der Auftrag bleiben dauerhaft technisch verfügbar; Akteurs-IDs werden nicht zusätzlich als ChangeEvent-Eigenschaften dupliziert. Auch das Anlegen dieser Auditdaten verwendet den gemeinsamen Commit-Gate.

Unter dem Gate prüft `SYNC` vor dem Fachcommit erneut den vollständigen Auftrag, Profilunterstützung, Antragsteller, Belege, Identität, Zeitgültigkeit, aktuelle Route, Befugnisse, RaN und alle übrigen Modellbedingungen. Geänderte Zielrevision, Policy, Accountability, Scope oder zusätzliche erforderliche Zweige machen eine frühere Genehmigung nicht automatisch weiterhin gültig. Abweichende Inhalte oder Grundlage benötigen einen neuen Auftrag und neue Bestätigungen; identische Wiederholungen dürfen nicht doppelt wirksam werden.

Erst `SUCCESS` übernimmt die Freigabe. Ein Task wird nach den vorhandenen Regeln `ACTIVE` oder bei unerfüllten Abhängigkeiten `BLOCKED`. Die Genehmigung erzeugt niemals unmittelbar `COMPLETED` oder `ACHIEVED`. `CONFLICT` oder `FAILED` übernimmt kein Fachdelta; die Genehmigungs- und Versuchsprovenienz bleibt erhalten. Eindeutige Folgeänderungen bleiben im selben `SyncRun`, ohne rekursive Genehmigungs- oder Ereigniskette.

#### 12.9.5 Anfangsbefugnis und Einführung

Die erste Wertentscheidung kann noch keine aktive RaN voraussetzen, die genau diese noch nicht entstandenen Werte schützt. Deshalb darf die Installation eine ausdrücklich von einem authentifizierten Menschen eingerichtete, technisch dokumentierte Anfangsbefugnis vorhalten: genaues menschliches Mitglied, konkrete Rollenaktivierung und konkreter Werteträger. Sie gilt ausschließlich für `Model.action.CONFIRM`, niemals für Task-Freigaben, und unterliegt denselben Identitäts-, Scope-, Hash- und Zeitprüfungen. Ohne solche ausdrücklich konfigurierte Befugnis bleibt die Entscheidung gesperrt. Das technische Root-`RoleAssignment` erhält dadurch keinerlei menschliche Entscheidungsbefugnis; der Sechs-Entitäten-Bootstrap bleibt unverändert.

Sobald für einen Werteträger erstmals eine aktive `VALUE_SCOPE`-Policy übernommen wurde, wird seine Anfangsbefugnis dauerhaft deaktiviert. Dieser technische Sperrmerker wird atomar mit dem Commit gespeichert und ist Teil der Freigabegrundlage. Ein späterer Widerruf oder die Entfernung einer Policy aktiviert ihn nicht erneut. Die Installation muss Registrierung und dauerhafte Sperre manipulationsgeschützt führen; dies ist kein automatischer Ersatz für eine fehlende Governance-Entscheidung.

Bestehende terminale Daten und Ereignisse erhalten keine erfundenen Genehmigungen. Fehlende historische Belege bleiben als solche sichtbar. Neue geschützte Vorgänge werden nur mit nachweislich profilfähiger SYNC-Definition und Handler-Paket angenommen. Die Referenzmodule sind ausführbare Entscheidungsprüfungen, keine produktive Authentifizierungs-, Nachrichten- oder Datenbank-Engine.

#### 12.9.6 Beispiel und Auswirkungen auf den Loop
...
Jedes `CiV` beschreibt einen einzelnen Wert mit NOT-, SELF- und TO-SERVE-Dimension und genau einem Werteträger. Ausdrücklich ausgewählte CiV begründen, warum eine Zukunft gewollt ist. `PiF2` bis `PiF1o` beschreiben diese Zukunft auf langfristiger, strategischer, taktischer und operativer Ebene. Die gespeicherten `CONTRIBUTES_TO`-Beziehungen führen dabei vom konkreteren zum übergeordneten Zukunftszustand und erlauben einen gerichteten `n:m`-Graphen.
...
```text
PiH ── PROVIDES_CONTEXT_TO ──► CiV ── HELD_BY ──► RoFOrg | RoFTeam | menschliches RoFTeamMember
                                      ├── INFORMED_BY ──► CiV
                                      └── INSCRIBES_PURPOSE_IN ──► PiF2
                                                                     ▲
...
RaN ── PROTECTS ──► CiV | PiF2
    └── GOVERNS ──► relevante Umsetzungselemente
