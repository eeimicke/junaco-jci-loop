...
`CiV` beschreibt Zweck und Werte. `PiF2` bis `PiF1o` übersetzen diesen Zweck in immer konkretere Zukunftszustände. Tasks realisieren den operativen Zustand. Ergebnisse werden geprüft. `RaN` begrenzt Entscheidungen, `RoF` ordnet Verantwortung zu und `ERoF` beschreibt die relevante Umwelt. `SYNC` verfolgt Änderungen; abgelöste Zustände bleiben als `PiH` erhalten.
...
- **Regeln als Querschnitt:** `RaN` kann konkrete Entscheidungen erlauben, verlangen oder verbieten.
